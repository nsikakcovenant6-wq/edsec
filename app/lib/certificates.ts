...
function certificateCode(year: number) {
  ...
  return `EDSEC-${year}-${suffix}`;
}

export async function listCertificateEligibleEnrollments() {
  return prisma.$queryRaw<EnrollmentRow[]>`
    SELECT e."id", CONCAT(u."firstName", ' ', u."lastName") AS "studentName",
           c."title" AS "courseName", e."status"
    FROM "Enrollment" e
...
export async function issueCertificate(enrollmentId: string) {
  ...
  if (enrollment.status !== "COMPLETED") {
    throw new Error("A certificate can only be issued for a completed enrollment.");
  }
...
export async function verifyCertificate(certificateId: string) {
  ...
  const rows = await prisma.$queryRaw<CertificateVerification[]>`
    SELECT cert."certificateId", cert."status",
           CONCAT(u."firstName", ' ', u."lastName") AS "studentName",
           c."title" AS "courseName", cert."issuedAt", cert."revokedAt",
