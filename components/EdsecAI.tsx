...
export default function EdsecAI() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      text: "Hi! I’m EDSEC AI 👋 Ask me about courses, fees, the Launch Promo, applications, login, certificates or certificate verification.",
    },
  ]);
  ...
  async function ask(question: string) {
    ...
    try {
      ...
      setMessages((current) => [
        ...current,
        { role: "assistant", text: data.answer || data.error || "I could not answer that right now." },
      ]);
    } catch {
      setMessages((current) => [...current, { role: "assistant", text: "EDSEC AI is temporarily unavailable. Please try again shortly." }]);
    } finally {
      setLoading(false);
    }
  }
        ...
        <section className="mb-3 flex h-[min(620px,calc(100vh-110px))] w-[min(390px,calc(100vw-24px))] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-950/20">
          <header className="flex items-center justify-between bg-slate-950 px-4 py-4 text-white">
            <div className="flex items-center gap-3">
              ...
              <div><p className="font-black">EDSEC AI</p><p className="text-[11px] text-slate-400">Admissions & information assistant</p></div>
            </div>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto bg-slate-50 p-4">
            {messages.map((message, index) => (
              <div key={`${message.role}-${index}`} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[88%] whitespace-pre-wrap rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === "user" ? "rounded-br-md bg-blue-600 text-white" : "rounded-bl-md border border-slate-200 bg-white text-slate-700"}`}>
                  {message.text}
                </div>
              </div>
            ))}
            ...
            {loading && <div className="flex items-center gap-2 text-xs font-semibold text-slate-400"><Loader2 className="h-4 w-4 animate-spin" />EDSEC AI is thinking…</div>}
          </div>

          <div className="border-t border-slate-200 bg-white p-3">
            <form onSubmit={submit} className="flex items-center gap-2">
              <input value={input} onChange={(event) => setInput(event.target.value)} maxLength={600} placeholder="Ask EDSEC AI…" aria-label="Ask EDSEC AI" className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10" />
            ...
            <div className="mt-2 flex items-center justify-between gap-3 text-[10px] text-slate-400"><span>Answers are based on verified EDSEC information.</span><Link href="/verify" className="font-bold text-blue-600">Verify certificates</Link></div>
          </div>
        </section>
      )}
